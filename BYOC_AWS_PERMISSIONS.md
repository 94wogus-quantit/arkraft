...
| **Tier-별도** | Cognito 미사용 | (현 prod 에서 Cognito Terraform 자원 0건 — JWT/OAuth 는 다른 메커니즘 추정. §10 출처/근거에서 재확인) |
...
| 24 | **IAM** | terraform + ArgoCD + Atlantis | Role/Policy/User/OIDC provider 관리 | global | `ai-infra/aws/iam*.tf` |
...
>
> **Resource: "\*" 사유 (Athena/Glue API)**: Athena `StartQueryExecution` 등은 workgroup-level scope 가능. 위 JSON 은 workgroup ARN narrow 후속 보강 예정 (iter 3).
