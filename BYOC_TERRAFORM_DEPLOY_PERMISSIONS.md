...
>
> **arkraft 한정 (B사 PoC 와 무관 — 모두 범위 밖)**:
> - **Quantit 환경 전용 자원** (BYOC_AWS_PERMISSIONS.md §1 의 제외 블록과 일치):
>   - Finter 외부 데이터 (`c2-performance-data-production` 외부 account read-only — Quantit 데이터 계약 전용)
>   - staging 환경 (`arkraft-staging` bucket / `arkraft-staging-sandbox` namespace — Quantit 만 운영)
>   - `arkraft.quantit.ai` 도메인 자원 (Quantit 매니저 사이트 + agent-manager IRSA — Quantit 내부)
> - **별도 시스템**: quanda / Atlantis (ECS) / EC2 Image Builder / SSM Bastion / WAFv2 / VPC Peering / OpenSearch Serverless / DynamoDB / Lambda alpha-migrator / agent-sandbox (Athena/Glue).
...
>
> **PoC 한정 narrow 사유**: `arn:aws:s3:::arkraft-production*` 으로 narrow — 이전 wildcard `arkraft-*` 는 Quantit 환경 전용 bucket (`arkraft-staging`, `arkraft.quantit.ai`) 까지 매치됐음. PoC 는 prod 한 환경이므로 production prefix 만 grant. B사 prefix 변경 시 ARN 도 같이 갱신.
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:ecr:ap-northeast-2:{ACCOUNT_ID}:repository/ark/arkraft-api",
        "arn:aws:ecr:ap-northeast-2:{ACCOUNT_ID}:repository/ark/arkraft-web",
        "arn:aws:ecr:ap-northeast-2:{ACCOUNT_ID}:repository/ark/arkraft-agent-alpha",
        "arn:aws:ecr:ap-northeast-2:{ACCOUNT_ID}:repository/ark/arkraft-agent-insight",
        "arn:aws:ecr:ap-northeast-2:{ACCOUNT_ID}:repository/ark/arkraft-agent-portfolio",
        "arn:aws:ecr:ap-northeast-2:{ACCOUNT_ID}:repository/ark/arkraft-agent-report",
        "arn:aws:ecr:ap-northeast-2:{ACCOUNT_ID}:repository/ark/arkraft-agent-extract",
        "arn:aws:ecr:ap-northeast-2:{ACCOUNT_ID}:repository/ark/arkraft-agent-data"
      ],
      "//comment": "PoC 한정 8 repos — agent-manager 는 Quantit 내부 (매니저 사이트 전용, 헤더 blockquote 의 제외 목록 참조) 라 PoC 에서 제외. 이전 wildcard ark/* 는 manager 까지 grant 했으나 정리됨."
    },
    {
      "Effect": "Allow",
      "Action": [
...
- [ ] §3.5 RDS PostgreSQL
- [ ] §3.6 ElastiCache Redis
- [ ] §3.7 S3 (arkraft-production* + ai-infra-argo-workflows-logs* — PoC 한정. staging bucket 은 Quantit 환경 전용으로 제외)
- [ ] §3.8 ECR (ark/arkraft-{api,web,agent-{alpha,insight,portfolio,report,extract,data}} 8 repos — agent-manager 는 Quantit 내부로 제외)
...
- **iter 2 (2026-04-28, PoC 한정 동기화 — `BYOC_AWS_PERMISSIONS.md` iter 10 와 framing 일치)**: PoC = B사 환경 한정으로, Quantit 환경 전용 자원/권한 모두 제거. AWS_PERMISSIONS 의 PoC cleanup 결과를 Terraform deploy 권한 측면에 미러링.
  - **헤더 blockquote (line 15~21)**: "arkraft 한정" 목록을 두 카테고리로 재구성 — (a) Quantit 환경 전용 자원 (Finter `c2-performance-data-production` / staging `arkraft-staging` bucket / `arkraft.quantit.ai` 도메인 자원 + agent-manager IRSA), (b) 별도 시스템 (quanda / Atlantis / WAFv2 등 기존 항목). AWS_PERMISSIONS §1 제외 블록과 동일한 자원 목록 + 사유 명시.
  - **§2 #10 S3 Bucket**: Resource ARN scope 정정 — `arn:aws:s3:::arkraft-*` (너무 넓어 Quantit 환경 전용 bucket 까지 매치) → `arn:aws:s3:::arkraft-production*` 로 narrow. 사유 컬럼에서 `arkraft-staging` 제거 + "PoC 한정 — staging bucket 은 Quantit 환경 전용으로 제외" 명시.
  - **§2 #11 ECR Repository**: 사유 컬럼 `arkraft 9 services` → `arkraft 8 services` (agent-manager 제외 명시 + 8 service list 인라인).
  - **§3.7 S3 Policy JSON**: Resource ARN `arn:aws:s3:::arkraft-*` → `arn:aws:s3:::arkraft-production*` narrow. PoC 한정 narrow 사유 blockquote 추가 — 이전 wildcard 가 Quantit 환경 전용 bucket 까지 grant 했음 명시.
  - **§3.8 ECR Policy JSON**: Resource `repository/ark/*` (manager 까지 grant) → 8 repo 명시적 list (api / web / agent-{alpha,insight,portfolio,report,extract,data}). `//comment` 로 manager 제외 사유 인라인.
  - **§7.2 Service 별 정책 attach 체크리스트**: §3.7 / §3.8 항목에 PoC 한정 narrow 표기 추가 — 보안팀이 어느 자원이 PoC 범위인지 한눈에 확인 가능.
  - **결과**: AWS_PERMISSIONS iter 10 와 framing 완전 일치. B사 보안팀이 두 문서를 차례로 읽었을 때 "PoC = B사 환경 / Quantit 자체 환경 자원은 제외" 메시지가 일관됨.
